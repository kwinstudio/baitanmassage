...
def treatments_section(site, treatments):
    ...
    for t in active_treatments(treatments):
        ...
        cards.append(f'''<article class="treatment treatment-visual"><a class="treatment-media" href="{esc(detail)}" data-treatment-modal="{esc(t.get('id'))}"><img src="{esc(image)}" alt="{esc(t.get('imageAlt') or t.get('name'))}" loading="lazy" decoding="async">{label_html}</a><div class="treatment-body"><div class="treatment-topline"><h3>{esc(t.get('name'))}</h3><div class="treatment-price">vanaf {money(start)}</div></div><p>{esc(t.get('description'))}</p><div class="treatment-meta">{meta}</div><div class="treatment-actions"><a class="text-link" href="{esc(detail)}" data-treatment-modal="{esc(t.get('id'))}">Bekijk behandeling <span>→</span></a><a class="text-link" href="{esc(booking_url)}"{external}>Afspraak maken <span>→</span></a></div></div></article>''')
