...
def reviews_section(site):
    r=site['reviews']
    return f'''<section class="section"><div class="container"><div class="review-band"><div><div class="eyebrow">Google Reviews</div><div class="rating-big">{esc(r.get('rating'))}</div><p class="rating-caption">{esc(r.get('count'))} Google-reviews</p></div><a class="btn btn-outline" href="{esc(r.get('url'))}" target="_blank" rel="noopener">{esc(r.get('button'))}</a></div></div></section>'''
...
def contact_section(site):
    ...
    return f'''<section class="section" id="contact"><div class="container contact-grid"><div class="contact-details"><div class="eyebrow">{esc(c.get('kicker'))}</div><h2>{esc(site.get('businessName'))}</h2><div class="detail-row"><span class="detail-label">Adres</span><div class="detail-value">{esc(site.get('addressLine1'))}<br>{esc(site.get('postalCity'))}</div></div><div class="detail-row"><span class="detail-label">Telefoon</span><div class="detail-value"><a href="tel:{esc(site.get('phoneHref'))}">{esc(site.get('phoneDisplay'))}</a></div></div><div class="detail-row"><span class="detail-label">E-mail</span><div class="detail-value"><a href="mailto:{esc(site.get('email'))}">{esc(site.get('email'))}</a></div></div><table class="hours" aria-label="Openingstijden"><tbody><tr><td>{esc(o.get('weekdayLabel'))}</td><td>{esc(o.get('weekdayOpen'))} — {esc(o.get('weekdayClose'))}</td></tr><tr><td>{esc(o.get('weekendLabel'))}</td><td>{esc(o.get('weekendOpen'))} — {esc(o.get('weekendClose'))}</td></tr></tbody></table><div class="contact-actions"><a class="btn" href="{esc(c.get('routeUrl'))}" target="_blank" rel="noopener">Route</a><a class="btn btn-outline" href="#boeken">Boek afspraak</a></div></div><iframe class="map" title="Kaart van Baitan Thai Massage" loading="lazy" referrerpolicy="no-referrer-when-downgrade" src="{esc(c.get('mapEmbedUrl'))}"></iframe></div></section>'''
...
def build():
    ...
    for name in ['styles.css','app.js','privacy.html','voorwaarden.html']:
